...
(function(){
  ...
  const HSI_INTRA="FOREXCOM:HK50";
  const HSI_DAILY="INDEX:HSI";
  const SYMS=[
    {id:"hsi", label:"HSI", kind:"hsi"},
    {id:"mini", label:"Mini", kind:"hsi"},
    {id:"HKEX:0700", label:"0700", kind:"eq"},
    {id:"HKEX:9988", label:"9988", kind:"eq"},
    {id:"HKEX:3690", label:"3690", kind:"eq"},
    {id:"HKEX:1810", label:"1810", kind:"eq"}
  ];
  function load(){ try{return JSON.parse(localStorage.getItem(LS)||"{}");}catch(e){return{};} }
  function save(o){ try{localStorage.setItem(LS,JSON.stringify(o));}catch(e){} }
  function intra(iv){ return iv!=="D" && iv!=="W" && iv!=="M"; }
  function resolve(pick, interval){
    const row=SYMS.find(s=>s.id===pick || s.label===pick);
    if(row && row.kind==="eq") return row.id;
    return intra(interval) ? HSI_INTRA : HSI_DAILY;
  }
  function st(){
    const s=load();
    let pick=s.pick||"HSI";
    if(s.symbol==="HKEX:MHI1!"||s.symbol==="HKEX:HSI1!"||s.symbol==="INDEX:HSI"||s.symbol==="HSI") pick=s.pick||"HSI";
    const interval=s.interval||"15";
    return {pick:pick, interval:interval, symbol:resolve(pick, interval)};
  }
  ...
  function html(){
    const cur=st();
    const btns=SYMS.map(s=>'<button type="button" class="'+(cur.pick===s.label||cur.pick===s.id?"on":"")+'" data-pick="'+s.label+'">'+s.label+'</button>').join("");
  ...
  function bind(el){
    el.querySelectorAll("[data-pick]").forEach(function(b){
      b.onclick=function(e){
        ...
        const s=st(); s.pick=b.getAttribute("data-pick"); s.symbol=resolve(s.pick,s.interval); save(s); place(true);
      };
    });
    const sel=el.querySelector("#tvHomeInt");
    if(sel) sel.onchange=function(){
      const s=st(); s.interval=this.value; s.symbol=resolve(s.pick,s.interval); save(s); place(true);
    };
  }
  ...
  function place(force){
    try{
      ...
      if(!el){ el=document.createElement("div"); el.id="cardTv"; }
